      ...
      {jnf.eligibility_rule && (
        <Card sx={{ mb: 3 }}>
          <CardContent sx={{ p: { xs: 2, md: 4 } }}>
            ...
            <Grid container spacing={2}>
              <Grid item xs={6} md={3}><Label>Min. CGPA</Label><Value>{jnf.eligibility_rule.min_cgpa ?? '—'}</Value></Grid>
              <Grid item xs={6} md={3}><Label>Max Backlogs</Label><Value>{jnf.eligibility_rule.max_backlogs_allowed ?? '—'}</Value></Grid>
              <Grid item xs={6} md={3}><Label>Gender</Label><Value>{jnf.eligibility_rule.allowed_gender ?? 'all'}</Value></Grid>
              <Grid item xs={6} md={3}><Label>Active Backlogs OK?</Label><Value>{jnf.eligibility_rule.active_backlogs_allowed ? 'Yes' : 'No'}</Value></Grid>
              {jnf.eligibility_rule.additional_text && (
                <Grid item xs={12}><Label>Additional Notes</Label><Value>{jnf.eligibility_rule.additional_text}</Value></Grid>
              )}
            </Grid>
          </CardContent>
        </Card>
          ...
          <CardContent sx={{ p: { xs: 2, md: 4 } }}>
            ...
            {jnf.salary_breakdowns.map((row: any) => (
              <Box key={row.id} sx={{ mb: 2, p: 2, border: '1px solid rgba(0,0,0,0.08)', borderRadius: 2 }}>
                <Typography sx={{ fontWeight: 600, fontSize: '0.85rem', color: '#003366', mb: 1 }}>
          ...
          <CardContent sx={{ p: { xs: 2, md: 4 } }}>
            ...
            {jnf.selection_rounds.map((round: any) => (
              <Box key={round.id} sx={{
                ...
                <Box>
                  <Typography sx={{ fontWeight: 600, fontSize: '0.9rem', textTransform: 'capitalize' }}>
